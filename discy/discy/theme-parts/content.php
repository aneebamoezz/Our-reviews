...
if ('post' !== $post_type || ('post' === $post_type && ($custom_permission != "on" || $is_super_admin || (is_user_logged_in() && isset($roles["show_post"]) && $roles["show_post"] == 1) || (!is_user_logged_in() && $show_post == "on") || ($user_id > 0 && $user_id == $post_author)))) {
	...
	$post_class .= ($post_style == "style_1"?" post-style-1":"");
				...
				<footer<?php echo ($pending_posts?" class='pending-post-footer'":"")?>>
					...
					if (has_wpqa() && $pending_posts) {
						...
						if ( (strpos(get_the_content(),'more-link') !== false || $read_more == "on") && !is_single() && (empty($blog_h) && (isset($wp_page_template) && ($wp_page_template == "template-blog.php" || $wp_page_template == "template-home.php")) || !is_page()) ) {?>
							<a<?php echo esc_attr($post_link_target)?> class="post-read-more" href="<?php echo esc_url(has_wpqa() && $post_type == "posts"?wpqa_custom_permalink($post_id,"view_posts_group","view_group_post"):get_permalink($post_id))?>" rel="bookmark" title="<?php esc_attr_e('Read','discy')?> <?php the_title()?>"><?php esc_html_e('Read more','discy')?></a>
						<?php }
