...
include locate_template("includes/".(isset($its_question) && (wpqa_questions_type == $its_question || wpqa_asked_questions_type == $its_question)?"question":"loop")."-setting.php");
...
if (empty($blog_h) && ((((isset($tab_category) && $tab_category == true) || (isset($tab_tag) && $tab_tag == true)) && ($first_one == $answers_slug || $first_one == $answers_slug_2))) || (isset($wp_page_template) && (($wp_page_template == "template-home.php" && is_user_logged_in() && isset($first_one) && $first_one != "" && ($first_one == $answers_might_like_slug || $first_one == $answers_for_you_slug || $first_one == $answers_might_like_slug_2 || $first_one == $answers_for_you_slug_2))) || (isset($wp_page_template) && (($wp_page_template == "template-home.php" && isset($first_one) && $first_one != "" && ($first_one == $answers_slug || $first_one == $answers_slug_2 || (!is_user_logged_in() && ($first_one == $answers_might_like_slug || $first_one == $answers_for_you_slug || $first_one == $answers_might_like_slug_2 || $first_one == $answers_for_you_slug_2)))) || $wp_page_template == "template-comments.php")))) {
	...
	if ($orderby_answers == 'votes' && ($post_type == wpqa_questions_type || $post_type == wpqa_asked_questions_type)) {
	...
	<?php }else {
		echo '<div class="alert-message warning"><i class="icon-flag"></i><p>'.($post_type == wpqa_questions_type || $post_type == wpqa_asked_questions_type?esc_html__("There are no answers yet.","discy"):esc_html__("There are no comments yet.","discy")).'</p></div>';
	}
	...
	if (isset($blog_h) && $blog_h == "blog_h") {
		...
		if ($loop_query == true) {
			...
			$active_sticky = $show_sticky = $post_not_true = true;
			$custom_args   = array("post_type" => wpqa_questions_type,"tax_query" => array(array("taxonomy" => wpqa_question_categories,"field" => "id","terms" => $get_tax->term_id)));
			...
			if (($first_one == $feed_slug && is_user_logged_in()) || ($first_one == $feed_slug_2 && is_user_logged_in()) || ($first_one == $poll_feed_slug && is_user_logged_in()) || ($first_one == $poll_feed_slug_2 && is_user_logged_in()) || (isset($wp_page_template) && $wp_page_template == "template-question.php" && $orderby_post == "feed" && is_user_logged_in())) {
				...
				if ($user_following_if == "yes" && $cat_following_if == "yes" && $tag_following_if == "yes" && ($all_following != "" || $user_following != "")) {
					...
					AND ($wpdb->posts.post_type = '".wpqa_questions_type."' ".apply_filters("discy_post_type_feed",false,$wpdb).")
					...
					if (is_array($query) && !empty($query)) :?>
						<div class="post-articles<?php echo (isset($its_question) && (wpqa_questions_type == $its_question || wpqa_asked_questions_type == $its_question)?" question-articles".(isset($question_columns) && $question_columns == "style_2" && isset($count_total) && $count_total > 0?" row row-boot row-warp".(isset($masonry_style) && $masonry_style == "on" && isset($count_total) && $count_total > 0?" isotope":""):""):"").($post_pagination == "none"?" no-pagination":"").(isset($blog_h) && $blog_h == "blog_h"?" post-articles-blog-h":"").(isset($post_style) && $post_style == "style_3"?" row row-boot row-warp":"")?>">
		...
		$post_not_true = true;
		$array_data = apply_filters("discy_args_question_category",array_merge($question_numbers,array("ignore_sticky_posts" => 1,"post_type" => wpqa_questions_type,"paged" => $paged,"tax_query" => array(array("taxonomy" => $tax_question,"field" => "id","terms" => $category_id,'operator' => 'IN')))),$tax_question,$paged,$question_numbers,$category_id);
		...
		$post_type_last_one = apply_filters("discy_question_posts_profile_post_type",$post_type_last_one,$last_one);
	...
	<section<?php echo ((isset($post_style) && $post_style == "style_3") || (isset($question_columns) && $question_columns == "style_2")?" class='section-post-with-columns'":"")?><?php echo ($last_one == "questions" || $last_one == "asked" || $last_one == "asked-questions" || $last_one == "paid-questions" || $last_one == "polls" || $last_one == "followed" || $last_one == "favorites" || $last_one == "followers-questions" || $last_one == "posts" || $last_one == "followers-posts" || (has_wpqa() && (wpqa_is_pending_questions() || wpqa_is_pending_posts()))?" id='section-".wpqa_user_title()."'":"")?>>
		<?php if ((isset($no_favorites) || $last_one == "favorites") && empty($array_favorites)) {
			...
			if (isset($its_question) && (wpqa_questions_type == $its_question || wpqa_asked_questions_type == $its_question)) {
				$page_tamplate = true;
			}
			...
			if (!isset($custom_sql)) :
				if (!empty($sticky_questions) || !empty($all_sticky_posts) || isset($custom_args) || (isset($query_wp) && $query_wp->have_posts()) || (have_posts() && empty($array_data))) :
					if (isset($blog_h) || empty($wp_page_template) || (isset($wp_page_template) && $wp_page_template != "template-users.php" && $wp_page_template != "template-contact.php" && $wp_page_template != "template-faqs.php" && $wp_page_template != "template-categories.php" && $wp_page_template != "template-tags.php")) :
						...
						<h2 class="screen-reader-text"><?php echo esc_html(get_bloginfo('name','display'))." ".esc_html__("Latest","discy")." ";printf("%s",(isset($its_question) && (wpqa_questions_type == $its_question || wpqa_asked_questions_type == $its_question)?esc_html__("Questions","discy"):esc_html__("Articles","discy")))?></h2>
						<div class="post-articles<?php echo (isset($its_question) && (wpqa_questions_type == $its_question || wpqa_asked_questions_type == $its_question)?" question-articles".(isset($question_columns) && $question_columns == "style_2" && isset($count_total) && $count_total > 0?" row row-boot row-warp".(isset($masonry_style) && $masonry_style == "on" && isset($count_total) && $count_total > 0?" isotope":""):""):"").($post_pagination == "none"?" no-pagination":"").(isset($blog_h) && $blog_h == "blog_h"?" post-articles-blog-h":"").(empty($more_link)?" articles-no-pagination":"").(isset($post_style) && $post_style == "style_3"?" row row-boot row-warp":"")?>">
					...
					if (!isset($no_feed_questions)) {
						if ((isset($blog_h) && $blog_h == "blog_h") || (isset($show_custom_error) && $show_custom_error == true && (!isset($is_questions_sticky) || (isset($is_questions_sticky) && $is_questions_sticky != true)))) {
							echo "<div class='alert-message warning'><i class='icon-flag'></i><p>".(isset($its_question) && ($its_question == wpqa_questions_type || $its_question == wpqa_asked_questions_type)?esc_html__("There are no questions yet.","discy"):esc_html__("There are no posts yet.","discy"))."</p></div>";
						}else {
