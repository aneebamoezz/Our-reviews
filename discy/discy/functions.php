...
if (!function_exists('discy_scripts_styles')) {
	function discy_scripts_styles() {
		...
		if (is_category() || is_tag() || (is_archive() && !is_post_type_archive(wpqa_questions_type) && !is_post_type_archive("group")) || is_tax(wpqa_question_categories) || $tax_filter == true || $tax_archive == true || is_tax(wpqa_question_tags) || is_post_type_archive(wpqa_questions_type) || is_post_type_archive("group") || $search_type == "groups" || $wpqa_group_id > 0) {
			if (is_category() || is_tag() || (is_archive() && !is_post_type_archive(wpqa_questions_type) && !is_post_type_archive("group") && $tax_archive != true && !is_tax(wpqa_question_tags))) {
			...
			if (is_tag() || (is_archive() && !is_category() && !is_tax(wpqa_question_categories) && $tax_filter == true && !is_post_type_archive(wpqa_questions_type) && !is_post_type_archive("group") && $tax_archive != true && !is_tax(wpqa_question_tags))) {
			...
			if (is_singular("post") || $search_type == "posts" || is_singular(wpqa_questions_type)) {
				$get_category = wp_get_post_terms($post->ID,(is_singular(wpqa_questions_type)?wpqa_question_categories:'category'),array("fields" => "ids"));
		...
		if ((is_category() || is_tag() || (is_archive() && !is_post_type_archive(wpqa_questions_type) && !is_post_type_archive("group")) || is_tax(wpqa_question_categories) || $tax_filter == true || $tax_archive == true || is_tax(wpqa_question_tags) || is_post_type_archive(wpqa_questions_type) || is_post_type_archive("group") || $search_type == "groups" || $wpqa_group_id > 0) && $primary_color_c == "") {
		...
		}else if ((is_category() || is_tag() || (is_archive() && !is_post_type_archive(wpqa_questions_type) && !is_post_type_archive("group")) || is_tax(wpqa_question_categories) || $tax_filter == true || $tax_archive == true || is_tax(wpqa_question_tags) || is_post_type_archive(wpqa_questions_type) || is_post_type_archive("group") || $search_type == "groups" || $wpqa_group_id > 0) && $primary_color_c != "") {
		...
		if (is_tax(wpqa_question_categories) || $show_category_cover != "") {
		...
		if ($date == "on") {
			...
			if ($format_date_ago == "on" && (($post_type == "post" && isset($format_date_ago_types["posts"]) && $format_date_ago_types["posts"] == "posts") || ((($post_type == wpqa_questions_type || $post_type == wpqa_asked_questions_type)) && isset($format_date_ago_types["questions"]) && $format_date_ago_types["questions"] == "questions"))) {
				if (wpqa_questions_type === $post_type || wpqa_asked_questions_type === $post_type) {
					...
					$data_string = esc_html__("Posted:","discy");
				}
			...
			$posted_on = (isset($data_string)?$data_string:'').'<span class="date-separator"></span> '.(wpqa_questions_type === $post_type || wpqa_asked_questions_type === $post_type?'<a href="'.get_the_permalink($post_id).'"'.(is_single()?' itemprop="url"':'').'>':'').$time_string.(wpqa_questions_type === $post_type || wpqa_asked_questions_type === $post_type?'</a>':'');
